...
CREATE OR REPLACE FUNCTION DEL_LOCACAO()
RETURNS TRIGGER AS 
$$

BEGIN

    DELETE FROM REL_LOCADORA 
...
/* QUAL A MODA DEPARTAMENTAL? QUAL DEPARTAMENTO MAIS EMPREGA? */

/* QUAL O DESVIO PADRAO DE CADA DEPARTAMENTO? */

/* CALCULE A MÉDIA SALARIAL DE TODO O SETOR DE DADOS */

/* QUAL A AMPLITUDE DE TODOS OS SALÁRIOS */

/* CALCULE AS PRINCIPAIS MEDIDAS ESTATÍSTICAS POR DEPARTAMENTO */

/* QUAL DEPARTAMENTO TE DÁ UMA MAIOR PROBABILIDADE DE GANAHR MAIS? */
...
/* UTILIZANDO COM VALORES BOLEANOS */

SELECT NOME, CARGO, (SEXO = 'Masculino') AS "MASCULINO", (SEXO = 'Feminino') AS "FEMINO"
FROM FUNCIONARIOS;
...
/* FILTROS BASEADOS EM MULTIPLAS COLUNAS E TIPOS - CONSIDERAR AND E OR */
...
/* FILTRO BASEADO EM PADRAO DE CARACTER */

SELECT DEPARTAMENTO, SUM(SALARIO) AS "TOTAL" 
FROM FUNCIONARIOS
WHERE DEPARTAMENTO LIKE 'B%' -- % É USADO COMO CORINGA, PODE SER USADO TANTO NO COMEÇO, MEIO E FIM.
GROUP BY DEPARTAMENTO;

/* COLUNAS NAO AGREGADAS COM COLUNAS AGREGADAS */
...
/* TRAZENDO SE A PESSOA É ASSISTENTE OU NÃO */
